...
def get_strategy_definitons():
    ...
    """
    strat_id_to_name = {0: "MACD_RSI", 1: "MACD_RSI_BOLL"}

    strat_id_to_TimeFrame = {0: TimeFrame(100, "1Day"), 1: TimeFrame(100, "1Day")}

    assert len(strat_id_to_name) == len(strat_id_to_TimeFrame), (
        "Lengths Mismatch: Add a strategy to both "
        "strat_name_to_id and strat_id_to_TimeFrame "
    )

    strat_id_to_lookback = {0: 100, 1: 100}
    ...
    assert len(strat_id_to_name) == len(
        inspect.getmembers(Strategies, inspect.isfunction)
    ), (
        "Lengths Mismatch: Add a strategy to both strat_name_to_id "
    ...
    strat_id_to_class = {
        id: getattr(strats, name.lower())(
            id, name, strat_id_to_TimeFrame[id], strat_id_to_lookback[id]
        )
