...
class MyPrompt(Cmd):
        ...
        if args == self.SHOW_COMMANDS[0]:  # strats
            ...
            x.add_rows(
                (
                    id,
                    name,
                    app.strat_id_to_class[id].length_of_data_needed,
                    app.strat_id_to_class[id].timeframe.datatype,
                )
    ...
    def do_track(self, args):
        ...
        if not VALID:
            print(
                'track needs a uid of an opportunity\nUse "show results" to find the uid'
            )
            return
        ...
        rows = [
            (
                ...
                TimeHandler.get_alpaca_string_from_datetime(
                    td["CURRENT_START_TIMESTAMP"]
                ),
            ),
            (
                "end_timestamp",
                TimeHandler.get_alpaca_string_from_datetime(
                    td["CURRENT_END_TIMESTAMP"]
                ),
            ),
        ]
    ...
    def check_set_integrity(self):
        ...
        if self.SET_DATA["strat"]:
            ...
            start_timestamp_string = TimeHandler.get_string_from_datetime(
                self.SET_DATA["timestamps"]["CURRENT_START_TIMESTAMP"]
            )
            end_timestamp_string = TimeHandler.get_string_from_datetime(
                self.SET_DATA["timestamps"]["CURRENT_END_TIMESTAMP"]
            )
            new_start, new_end, date_range = dmgr.validate_timestamps(
                start_timestamp_string, end_timestamp_string
            )
            (
                self.SET_DATA["timestamps"]["CURRENT_START_TIMESTAMP"],
                self.SET_DATA["timestamps"]["CURRENT_END_TIMESTAMP"],
            ...
            if not (len(date_range) >= strat.length_of_data_needed):
                ...
                assumed_start = (
                    self.SET_DATA["timestamps"]["CURRENT_END_TIMESTAMP"]
                    - dt.timedelta(int(strat.length_of_data_needed * 2))
                ).date()
                assumed_start_string = TimeHandler.get_string_from_datetime(
                    assumed_start
                )
                _, _, date_range = dmgr.validate_timestamps(
                    assumed_start_string, new_end
                )
                new_start_timestamp = date_range[-strat.length_of_data_needed]
                new_start_datetime = TimeHandler.get_datetime_from_timestamp(
                    new_start_timestamp
                )
                self.SET_DATA["timestamps"][
                    "CURRENT_START_TIMESTAMP"
                ] = new_start_datetime
                print(
                    f"WARNING: start_timestamp was changed to {TimeHandler.get_string_from_datetime(new_start_datetime)} to accomodate for strategy"
                )
        ...
        if not INTEGRITY:
            msg = (
                "SET ERROR(s): Run the set command again to resolve integrity issues\n"
                + msg
            )
    ...
    def do_timestamps(self, args):
        ...
        if START_FLAG:
            while 1:
                print(
                    "start_timestamp",
                    TimeHandler.get_alpaca_string_from_datetime(start_datetime),
                    end=" ",
                )
                user_inp = str(input())
                if user_inp == "":
        ...
        if END_FLAG:
            while 1:
                print(
                    "end_timestamp",
                    TimeHandler.get_alpaca_string_from_datetime(end_datetime),
                    end=" ",
                )
                user_inp = str(input())
                if user_inp == "":
    ...
    def show_untracked(self):
        ...
        position_list = []
        for pickle_file_name in list_pickle_file_names:
            pos: Position = Position.depickle(
                paths.untracked_trades_path, os.path.splitext(pickle_file_name)[0]
            )
    ...
    def do_untrack(self, args):
        ...
        shutil.move(
            os.path.join(paths.tracked_trades_path, file_name),
    ...
    def completions_list(self, text, list_of_completions):
        if not text:
            ...
            completions = [f for f in list_of_completions if f.startswith(text)]
        return completions
