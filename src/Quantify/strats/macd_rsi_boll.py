...
class Macd_Rsi_Boll(BaseStrategy):
    ...
    def _score(self, input_df: DataFrame):
        ...
        input_df["c_shifted_rsi"] = input_df.apply(
            lambda row: IndicUtils.constrain(row["shifted rsi"], 20, 80), axis=1
        )
        input_df["zc_shifted_rsi"] = input_df.apply(
            lambda row: IndicUtils.nan_out_between_range(row["c_shifted_rsi"], 40, 60),
            axis=1,
        )
        ...
        input_df["rsi_health_score"] = input_df.apply(
            lambda row: IndicUtils.account_for_buy_sell_signal(
                row["shifted rsi"] / 100, row["buy/sell signal"]
            ),
            axis=1,
        )
        input_df["rsi_score"] = input_df.apply(
            lambda row: IndicUtils.account_for_buy_sell_signal(
                row["zc_shifted_rsi"] / 100, row["buy/sell signal"]
            ),
            axis=1,
        )
