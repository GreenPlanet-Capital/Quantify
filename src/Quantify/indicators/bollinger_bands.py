...
class BollingerBands(BaseIndicator):
    ...
    def run(self, input_dataframe: DataFrame):
        ...
        simple_moving_avg = (
            input_dataframe["close"].rolling(window=self.num_periods).mean()
        )
        ...
        self.insert_all_into_df(
            input_dataframe, boll_bands, ["upper_bb", "lower_bb", "diff_bb", "sma_bb"]
        )

    def graph(self, input_df: DataFrame):
        input_df["close"].plot(label="Close prices", color="skyblue")
        input_df["upper_bb"].plot(
            label="Upper BB", linestyle="--", linewidth=1, color="black"
        )
        input_df["sma_bb"].plot(
            label="Middle BB", linestyle="--", linewidth=1.2, color="grey"
        )
        input_df["lower_bb"].plot(
            label="Lower BB", linestyle="--", linewidth=1, color="black"
        )
